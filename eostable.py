...
#
    def loadh2osesame(self, fname, unitstxt=None):
        """Function for loading H2O SESAME-STYLE EOS table output from Stewart calculation"""
        data = ([])
        if unitstxt is None:
            self.units = 'Units: rho kg/m3, T K, P MPa, U kJ/kg, S kJ/K/kg, 2D arrays are (NT,ND).'
        else:
            self.units = unitstxt
        sesamefile = open(fname,"r")  
        sesamedata=sesamefile.readlines()
        ...
        self.S   = data[2+self.ND+self.NT+2*self.ND*self.NT
                            : 2+self.ND+self.NT+3*self.ND*self.NT
                            ].reshape(self.NT,self.ND)
        #self.S   = data[2+self.ND+self.NT+3*self.ND*self.NT
        #                    : 2+self.ND+self.NT+4*self.ND*self.NT
            ...
            if testout[i] == '  TWO-PHASE BOUNDARIES\n':
                ...
                while flag == 0:
                    if testout[j+i+4].find(' anphas') == 0:
                        ...
                        #print(tmp3)
                        tmp4 = list(tmp3.split())
                        if (len(tmp4) ==0): # if the number is formatted badly, stop reading in the vapor curve, e.g. 1.00000-321
